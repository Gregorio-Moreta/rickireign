...
- **Local `tsc` / `next build` FALSE-FAIL on `cloudflare/worker.ts` when `.open-next/worker.js` is absent.** The tsconfig `exclude` of `cloudflare/worker.ts` does NOT actually keep it out of the local `tsc`/`next build` program (verified on clean `main`); the build only passes when a real `.open-next/worker.js` exists (produced by a prior `opennextjs-cloudflare build`). A fresh clone — or a tree right after `rm -rf .open-next` — red-herring-fails with `Cannot find module '../.open-next/worker.js'` at `cloudflare/worker.ts:8`. **Fix: run `npx opennextjs-cloudflare build` first**, then `tsc`/`next build` are clean. (This supersedes the 007 note that implied the exclude prevents the type-check — it doesn't; the artifact's presence is what matters. `opennextjs-cloudflare build` itself cleans `.open-next` before its internal `next build`, so you can't pre-seed a stub — but the CF build regenerates the real worker at the end and completes if the type-check passes; to measure size from a clean tree, temporarily set `typescript.ignoreBuildErrors` in `next.config.ts`, run the CF build, then `git checkout next.config.ts`.)
...
0 Foundation ✓ · 1 Content model ✓ · 2 Home ✓ (PR #4) · 3 Forms & legal ✓ (PR #5 + fix-round PR #6) · 4 Blog ✓ (PR #7) · 5 Verify & ship ✓ (PR #8). All six build phases complete. Post-launch work lands on numbered branches that are **not** build phases: **`006-home-reframe` ✓ (PR #9, merged)** · **`007-cf-sentry-bundle-fix` ✓ (PR #10, merged — CF Worker back under the 3 MiB limit via the @sentry/cloudflare split; both prod targets green)** · **`008-content-to-sanity` ✓ (PR open, awaiting sign-off — moved UI microcopy into Sanity: `siteSettings.consent`, `homePage.{newsletter,connect}.form`, new `journalPage` singleton; `siteSettings.seo` now drives layout `generateMetadata`).** **Next:** apply Ricki's copy-survey answers (mostly Studio edits) once she returns `docs/planning/ricki-copy-survey.docx`, or pick a launch-prep item. See `docs/SESSION_STATE.md`. **Identity wording still pending Ricki (Survey Q3):** `siteSettings.footerText`/`seo.title` keep the OLD "Founder, Facilitator & Organizational Leader"; the SEO field is now wired so flipping it to "leadership strategist" is a no-code Studio edit once she decides.
