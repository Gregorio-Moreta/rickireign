...
0 Foundation ✓ · 1 Content model ✓ · 2 Home ✓ (PR #4) · 3 Forms & legal ✓ (PR #5 + fix-round PR #6) · 4 Blog ✓ (PR #7) · 5 Verify & ship ✓ (PR #8). All six build phases complete. Post-launch work lands on numbered branches that are **not** build phases: **`006-home-reframe` ✓ (PR #9, merged)** · **`007-cf-sentry-bundle-fix` ✓ (PR #10, merged — CF Worker back under the 3 MiB limit via the @sentry/cloudflare split; both prod targets green).** **Next: `008-content→Sanity`** — move remaining hardcoded copy into Sanity so Ricki can edit it in the Studio, driven by the copy survey (`docs/planning/ricki-copy-survey.md`). Layout is settled; this is content-plumbing + copy, not a redesign. See `docs/SESSION_STATE.md`. **Known live copy bug:** `siteSettings.footerText`/`seo.title` still say the OLD identity ("Founder, Facilitator & Organizational Leader") vs. the reframed "leadership strategist" — reconcile in 008 (editable in Studio, no code change).
