...
- **Tags are a preset list** (`POST_TAGS` in `studio/schemaTypes/documents/post.ts`), so `/blog/tag/<slug>` pages group consistently (no free-text drift). Add a tag by appending `POST_TAGS` + `npm run schema:deploy` + `npx sanity deploy`. For no-code editor-managed tags, promote to a reference `tag` document type (deferred).
